...
@Service("ChangeTypeBalance1Process")
@ConfigurationProperties(prefix = "process.jde.send")
@RequiredArgsConstructor
@Slf4j
public class ChangeTypeBalance1Process implements IProcess{
	...
	public void run() throws ProcessException {
	    ...
	    try {
			...
			auditoria.setAction(AuditoriaConstans.ACTION_ORPAK_CHANGE_TYPE_BALANCE);
			auditoria.setIdElemento(0L);
			auditoria.setTipoElemento(AuditoriaConstans.TIPO_ELEMENTO_DEPARTAMENTO);
			auditoria.setDescription("Inicio proceso ChangeTypeBalance1Process (UTBL1)");
	        ...
	        safCriterio.setType(SafConstants.SAF_CONDITIONAL_1);
	        ...
	        if (pendingMessages != null && !pendingMessages.isEmpty()) {
				...
				auditoriaDos.setAction(AuditoriaConstans.ACTION_ORPAK_CHANGE_TYPE_BALANCE);
	            ...
	            for (MessageDTO message : pendingMessages) {
	                ...
	                try {
	                    procesoOrpak.updTypeBalance1(message);
	                    successCount++;
			...
			auditoriaCinco.setAction(AuditoriaConstans.ACTION_ORPAK_CHANGE_TYPE_BALANCE);
			auditoriaCinco.setIdElemento(0L);
			auditoriaCinco.setTipoElemento(AuditoriaConstans.TIPO_ELEMENTO_DEPARTAMENTO);
			auditoriaCinco.setDescription(String.format("Fin proceso ChangeTypeBalance1Process, Total registros enviados: %d, tiempo de ejecución: %d ms",successCount, Duration.between(startTime, endTime).toMillis()));
