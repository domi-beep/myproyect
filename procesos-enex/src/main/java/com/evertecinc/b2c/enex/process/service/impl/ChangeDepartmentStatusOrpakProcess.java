...
@Service("ChangeDepartmentStatusOrpakProcess")
@ConfigurationProperties(prefix = "process.jde.send")
@RequiredArgsConstructor
@Slf4j
public class ChangeDepartmentStatusOrpakProcess implements IProcess {
	...
	public void run() throws ProcessException {
	    ...
	    log.info("-----------------------------------------------");
	    log.info("Inicio proceso ChangeDepartmentStatusOrpakProcess");
	    ...
	    try {
			...
			auditoria.setDescription("Inicio proceso cambio estado departamento (USD) 2");
	        ...
	        if (pendingMessages != null && !pendingMessages.isEmpty()) {
	            ...
	            for (MessageDTO message : pendingMessages) {
	                ...
	                try {
	                    procesoOrpak.updPendingDepartmentStatusChange(message);
	                    successCount++;
			...
			auditoriaTres.setDescription(String.format("Fin proceso cambio estado departamento, Total registros enviados: %d, tiempo de ejecución: %d ms",successCount, Duration.between(startTime, endTime).toMillis()));
