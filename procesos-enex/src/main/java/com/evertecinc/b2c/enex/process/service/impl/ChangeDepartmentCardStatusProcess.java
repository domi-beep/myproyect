...
@Service("ChangeDepartmentCardStatusProcess")
@ConfigurationProperties(prefix = "process.jde.send")
@RequiredArgsConstructor
@Slf4j
public class ChangeDepartmentCardStatusProcess implements IProcess {
	...
	public void run() throws ProcessException {
	    ...
	    log.info("-----------------------------------------------");
	    log.info("Inicio proceso ChangeDepartmentCardStatusProcess");
	    ...
	    try {
			...
			auditoria.setAction(AuditoriaConstans.ACTION_ORPAK_CHANGE_DEP_STATUS);
			auditoria.setIdElemento(0L);
			auditoria.setTipoElemento(AuditoriaConstans.TIPO_ELEMENTO_DEPARTAMENTO);
			auditoria.setDescription("Inicio proceso cambio estado departamento (USD)");
	        ...
	        safCriterio.setType(SafConstants.SAF_UPDATE_DEPARTMENT_STATUS);
	        ...
	        if (pendingMessages != null && !pendingMessages.isEmpty()) {
	            ...
	            auditoriaDos.setAction(AuditoriaConstans.ACTION_ORPAK_CHANGE_DEP_STATUS);
	            ...
	            for (MessageDTO message : pendingMessages) {
	                ...
	                try {
	                    procesoOrpak.updPendingDepartmentCardStatusChange(message);
	                    successCount++;
	        ...
	        auditoriaTres.setAction(AuditoriaConstans.ACTION_ORPAK_CHANGE_DEP_STATUS);
	        auditoriaTres.setIdElemento(0L);
	        auditoriaTres.setTipoElemento(AuditoriaConstans.TIPO_ELEMENTO_DEPARTAMENTO);
	        auditoriaTres.setDescription(String.format("Fin proceso envío de cambio estado departamento, Total registros enviados: %d, tiempo de ejecución: %d ms",successCount, Duration.between(startTime, endTime).toMillis()));
