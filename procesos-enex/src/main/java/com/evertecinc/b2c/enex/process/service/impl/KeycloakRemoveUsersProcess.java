...
@Service("KeycloakRemoveUsersProcess")
//@ConfigurationProperties(prefix = "process.jde.send")
@RequiredArgsConstructor
@Slf4j
public class KeycloakRemoveUsersProcess implements IProcess {
	...
	public void run() throws ProcessException {
	    ...
	    log.info("-----------------------------------------------");
	    log.info("Inicio proceso eliminación usuarios keycloak");
		...
		auditoria.setAction(AuditoriaConstans.ACTION_KEYCLOAK_DELETE_USER);
		auditoria.setIdElemento(0L);
		auditoria.setTipoElemento(AuditoriaConstans.TIPO_ELEMENTO_USUARIO);
		auditoria.setDescription("Inicio proceso KeycloakRemoveUsersProcess");
		...
		try {
	        ...
	        auditoriaCinco.setAction(AuditoriaConstans.ACTION_KEYCLOAK_DELETE_USER);
	        auditoriaCinco.setIdElemento(0L);
	        auditoriaCinco.setTipoElemento(AuditoriaConstans.TIPO_ELEMENTO_USUARIO);
	        auditoriaCinco.setDescription(String.format("Fin proceso KeycloakRemoveUsersProcess, tiempo de ejecución: %d ms", Duration.between(startTime, endTime).toMillis()));
			...
			log.error("Error al eliminar usuario KC: "+e.getMessage());
	        ...
	        auditoriaDos.setAction(AuditoriaConstans.ACTION_KEYCLOAK_DELETE_USER);
