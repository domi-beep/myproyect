...
@Service("CreateMailReestablecerClaveProcess")
//@ConfigurationProperties(prefix = "process.jde.send")
@RequiredArgsConstructor
@Slf4j
public class CreateMailReestablecerClaveProcess implements IProcess{
	...
	public void run() throws ProcessException {
	    ...
	    log.info("-----------------------------------------------");
	    log.info("Inicio proceso CreateMailReestablecerClaveProcess");

	    try {
	    	log.info("Recuperar registros pendientes SMRPK (Send Mail Reestablecer Clave Keycloak)");
	    	
	    	SafCriterioDTO safCriterio = new SafCriterioDTO();
	    	safCriterio.setType(SafConstants.SAF_SEND_MAIL_REESTABLECER_PASSWORD_KEYCLOAK);
	        ...
	        if(safRecuperada == null || safRecuperada.size() <= 0) {
	        	log.info("No hay saf a recuperar para el proceso CreateMailReestablecerClaveProcess");
	        	return;
	        }
			...
			auditoria.setAction(AuditoriaConstans.ACTION_SEND_MAIL_RST_PWD);
			auditoria.setIdElemento(0L);
			auditoria.setTipoElemento(AuditoriaConstans.TIPO_ELEMENTO_MAIL);
			auditoria.setDescription("Se comienzan a procesar correos de restablcer contraseña "+safRecuperada.size());
	        ...
	        for(SafQueue saf : safRecuperada) {
	        	...
	        	try {
	        		...
	        		SafCorreoReestablecerPassword safData = new SafCorreoReestablecerPassword();
			        
			        safData = gson.fromJson(saf.getData(), SafCorreoReestablecerPassword.class);
			        ...
			        this.emailService.SendMailReestablecerClaveKeycloak(safData);
	        ...
	        auditoriaCinco.setAction(AuditoriaConstans.ACTION_SEND_MAIL_RST_PWD);
	        auditoriaCinco.setIdElemento(0L);
	        auditoriaCinco.setTipoElemento(AuditoriaConstans.TIPO_ELEMENTO_MAIL);
	        auditoriaCinco.setDescription(String.format("Fin proceso correos restablecer clave, Total registros enviados: %d, tiempo de ejecución: %d ms",successCount, Duration.between(startTime, endTime).toMillis()));
