...
@Service("ChangeTypeBalance2LitrosProcess")
@ConfigurationProperties(prefix = "process.jde.send")
@RequiredArgsConstructor
@Slf4j
public class ChangeTypeBalance2LitrosProcess implements IProcess {
	...
	public void run() throws ProcessException {
	    ...
	    log.info("-----------------------------------------------");
	    log.info("Inicio proceso ChangeTypeBalance2LitrosProcess");
	    ...
	    try {
			...
			auditoria.setAction(AuditoriaConstans.ACTION_ORPAK_CHANGE_TYPE_BALANCE_LITROS_2);
			auditoria.setIdElemento(0L);
			auditoria.setTipoElemento(AuditoriaConstans.TIPO_ELEMENTO_DEPARTAMENTO);
			auditoria.setDescription("Inicio proceso ChangeTypeBalance2LitrosProcess (UTBL2)");
	        ...
	        safCriterio.setType(SafConstants.SAF_CONDITIONAL_LT_2);
	        ...
	        if (pendingMessages != null && !pendingMessages.isEmpty()) {
	            ...
	            auditoriaDos.setAction(AuditoriaConstans.ACTION_ORPAK_CHANGE_TYPE_BALANCE_LITROS_2);
	            ...
	            for (MessageDTO message : pendingMessages) {
	                ...
	                try {
	                    procesoOrpak.updTypeBalance2Litros(message);
	                    successCount++;
	        ...
	        auditoriaCinco.setAction(AuditoriaConstans.ACTION_ORPAK_CHANGE_TYPE_BALANCE_LITROS_2);
	        ...
	        auditoriaCinco.setDescription(String.format(
	                "Fin proceso ChangeTypeBalance2LitrosProcess, Total registros enviados: %d, tiempo de ejecución: %d ms",
