...
@Service("ChangeTypeBalance1LitrosProcess")
@ConfigurationProperties(prefix = "process.jde.send")
@RequiredArgsConstructor
@Slf4j
public class ChangeTypeBalance1LitrosProcess implements IProcess {
	...
	public void run() throws ProcessException {
	    ...
	    log.info("-----------------------------------------------");
	    log.info("Inicio proceso ChangeTypeBalance1LitrosProcess");
	    ...
	    try {
			...
			auditoria.setAction(AuditoriaConstans.ACTION_ORPAK_CHANGE_TYPE_BALANCE_LITROS);
			auditoria.setIdElemento(0L);
			auditoria.setTipoElemento(AuditoriaConstans.TIPO_ELEMENTO_DEPARTAMENTO);
			auditoria.setDescription("Inicio proceso ChangeTypeBalance1LitrosProcess (UTBL1)");
	        ...
	        safCriterio.setType(SafConstants.SAF_CONDITIONAL_LT_1);
	        ...
	        if (pendingMessages != null && !pendingMessages.isEmpty()) {
				...
				auditoriaDos.setAction(AuditoriaConstans.ACTION_ORPAK_CHANGE_TYPE_BALANCE_LITROS);
	            ...
	            for (MessageDTO message : pendingMessages) {
	                ...
	                try {
	                    procesoOrpak.updTypeBalance1Litros(message);
	                    successCount++;
			...
			auditoriaCinco.setAction(AuditoriaConstans.ACTION_ORPAK_CHANGE_TYPE_BALANCE_LITROS);
			auditoriaCinco.setIdElemento(0L);
			auditoriaCinco.setTipoElemento(AuditoriaConstans.TIPO_ELEMENTO_DEPARTAMENTO);
			auditoriaCinco.setDescription(String.format("Fin proceso ChangeTypeBalance1LitrosProcess, Total registros enviados: %d, tiempo de ejecución: %d ms",successCount, Duration.between(startTime, endTime).toMillis()));
