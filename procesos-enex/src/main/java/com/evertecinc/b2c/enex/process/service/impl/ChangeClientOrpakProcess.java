...
@Service("ChangeClientOrpakProcess")
@ConfigurationProperties(prefix = "process.jde.send")
@RequiredArgsConstructor
@Slf4j
public class ChangeClientOrpakProcess implements IProcess {
	...
	public void run() throws ProcessException {
	    ...
	    log.info("-----------------------------------------------");
	    log.info("Inicio proceso ChangeClientOrpakProcess");
	    ...
	    try {
			...
			auditoria.setAction(AuditoriaConstans.ACTION_ORPAK_CHANGE_CLIENT);
			auditoria.setIdElemento(0L);
			auditoria.setTipoElemento(AuditoriaConstans.TIPO_ELEMENTO_CLIENTE);
			auditoria.setDescription("Inicio proceso cambios cliente Orpak (UCO)");
	        ...
	        safCriterio.setType(SafConstants.SAF_UPDATE_CLIENT_ORPAK);
	        ...
	        safCriterio2.setType(SafConstants.SAF_CHANGE_CUSTOMER_STATUS);
	        safCriterio2.setStatus(SafConstants.SAF_STATUS_PENDING);
			List<MessageDTO> pendingMessages2 = safService.getSafDTO(safCriterio2 );
			
			SafCriterioDTO safCriterio3 = new SafCriterioDTO();
	        safCriterio3.setType(SafConstants.SAF_CHANGE_CUSTOMER_CREDITLIMIT);
	        safCriterio3.setStatus(SafConstants.SAF_STATUS_PENDING);
			List<MessageDTO> pendingMessages3 = safService.getSafDTO(safCriterio3 );
	        ...
	        if (pendingMessages != null && !pendingMessages.isEmpty()) {
				...
				auditoriaDos.setAction(AuditoriaConstans.ACTION_ORPAK_CHANGE_CLIENT);
	            ...
	            for (MessageDTO message : pendingMessages) {
	                ...
	                try {
	                    procesoOrpak.updateClient(message);
	                    successCount++;
			...
			auditoriaTres.setAction(AuditoriaConstans.ACTION_ORPAK_CHANGE_CLIENT);
			auditoriaTres.setIdElemento(0L);
			auditoriaTres.setTipoElemento(AuditoriaConstans.TIPO_ELEMENTO_CLIENTE);
			auditoriaTres.setDescription(String.format("Fin proceso envío de cambios cliente Orpak, Total registros enviados: %d, tiempo de ejecución: %d ms",successCount, Duration.between(startTime, endTime).toMillis()));
