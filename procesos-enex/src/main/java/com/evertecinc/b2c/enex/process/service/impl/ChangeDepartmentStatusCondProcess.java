...
@Service("ChangeDepartmentStatusCondProcess")
@ConfigurationProperties(prefix = "process.jde.send")
@RequiredArgsConstructor
@Slf4j
public class ChangeDepartmentStatusCondProcess implements IProcess {
	...
	public void run() throws ProcessException {
	    ...
	    log.info("-----------------------------------------------");
	    log.info("Inicio proceso ChangeDepartmentStatusCondProcess");
	    ...
	    try {
			...
			auditoria.setAction(AuditoriaConstans.ACTION_ORPAK_CHANGE_DEP_STATUS_COND);
			auditoria.setIdElemento(0L);
			auditoria.setTipoElemento(AuditoriaConstans.TIPO_ELEMENTO_DEPARTAMENTO);
			auditoria.setDescription("Inicio proceso cambio estado conditional departamento (UTB3,UTBL3,UTB4,UTBL4)");
	        ...
	        safCriterio.setType(SafConstants.SAF_CONDITIONAL_3);
	        ...
	        safCriterio2.setType(SafConstants.SAF_CONDITIONAL_4);
	        safCriterio2.setStatus(SafConstants.SAF_STATUS_PENDING);
			List<MessageDTO> pendingMessages2 = safService.getSafDTO(safCriterio);
			
			SafCriterioDTO safCriterio3 = new SafCriterioDTO();
	        safCriterio3.setType(SafConstants.SAF_CONDITIONAL_LT_3);
	        safCriterio3.setStatus(SafConstants.SAF_STATUS_PENDING);
			List<MessageDTO> pendingMessages3 = safService.getSafDTO(safCriterio);
			
			SafCriterioDTO safCriterio4 = new SafCriterioDTO();
	        safCriterio4.setType(SafConstants.SAF_CONDITIONAL_LT_4);
	        safCriterio4.setStatus(SafConstants.SAF_STATUS_PENDING);
			List<MessageDTO> pendingMessages4 = safService.getSafDTO(safCriterio);
			
			pendingMessages.addAll(pendingMessages2);
			pendingMessages.addAll(pendingMessages3);
			pendingMessages.addAll(pendingMessages4);
	        ...
	        if (pendingMessages != null && !pendingMessages.isEmpty()) {
				...
				auditoriaDos.setAction(AuditoriaConstans.ACTION_ORPAK_CHANGE_DEP_STATUS_COND);
	            ...
	            for (MessageDTO message : pendingMessages) {
	                ...
	                try {
	                    procesoOrpak.updChangeDepartmentStatusCond(message);
	                    successCount++;
			...
			auditoriaTres.setAction(AuditoriaConstans.ACTION_ORPAK_CHANGE_DEP_STATUS_COND);
			auditoriaTres.setIdElemento(0L);
			auditoriaTres.setTipoElemento(AuditoriaConstans.TIPO_ELEMENTO_DEPARTAMENTO);
			auditoriaTres.setDescription(String.format("Fin proceso cambio estado conditional departamento, Total registros enviados: %d, tiempo de ejecución: %d ms",
